...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query = self.split(self.W_query(query))  # [batch_size, output_sequence_length, nhead, h_dim]
        key = self.split(self.W_key(key))  # [batch_size, input_sequence_length, nhead, h_dim]
        value = self.split(self.W_value(value))  # [batch_size, input_sequence_length, nhead, h_dim]
    ...
    def scaled_dot_product(self, query, key, value, mask=None):
        ...
        # query: [batch_size, output_sequence_length, nhead, h_dim]
        # key: [batch_size, output_sequence_length, nhead, h_dim]
        score = torch.einsum('bihd,bkhd->bhik', query, key)  # [batch_size, nhead, output_sequence_length, input_sequence_length]
        score = score * (1 / torch.sqrt(d_key))

        if mask is not None:
            mask = mask.unsqueeze(dim=0).unsqueeze(dim=0).repeat(batch_size, self.nhead, 1, 1)
            score = score.masked_fill(mask == 0, -10000)
        ...
        output = torch.einsum('bhij,bjhk->bihk', score, value)  # [batch_size, output_sequence_length, nhead, h_dim]
        return output
